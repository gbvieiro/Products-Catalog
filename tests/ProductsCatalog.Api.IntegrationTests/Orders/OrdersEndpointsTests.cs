...
public class OrdersEndpointsTests(ApiWebApplicationFactory factory) : IClassFixture<ApiWebApplicationFactory>
{
    ...
    public async Task FullOrderLifecycle_ReservesAndReplenishesStock()
    {
        ...
        var createOrderResponse = await _client.PostAsJsonAsync(
            "/api/orders", new CreateOrderCommand(Guid.NewGuid(), [new CreateOrderItemRequest(bookId, 3)]));
    ...
    public async Task CreateOrder_WithoutEnoughStock_ReturnsBadRequest()
    {
        ...
        var response = await _client.PostAsJsonAsync(
            "/api/orders", new CreateOrderCommand(Guid.NewGuid(), [new CreateOrderItemRequest(bookId, 1)]));
